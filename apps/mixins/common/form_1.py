...
from websaw.core import request, HTTP
...
class SelectWidget:
    def make(self, field, value, error, title, placeholder="", readonly=False):
        ...
        control.append(
            LINK(
                _href="https://cdn.jsdelivr.net/npm/select2@4.1.0-rc.0/dist/css/select2.min.css",
                _rel="stylesheet"
            )
        )
        control.append(
            SCRIPT('',
                   _src="https://ajax.googleapis.com/ajax/libs/jquery/3.5.1/jquery.min.js"
                   )
        )
        control.append (
            SCRIPT('',
               _src="https://cdn.jsdelivr.net/npm/select2@4.1.0-rc.0/dist/js/select2.min.js"
            )
        )
        control.append(
            SCRIPT(
...
class Form(object):
    ...
    def _make_key(self):
        if self.csrf_session is not None:
            ...
            key = str(uuid.uuid1())
            self.ctx.response.set_cookie("_formkey", key, same_site="Strict")
        return key
    ...
    def _get_key(self):
        if self.csrf_session is not None:
            ...
            key = self.ctx.request.get_cookie("_formkey")
        return key
