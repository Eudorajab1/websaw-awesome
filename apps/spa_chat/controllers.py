...
def index(ctx: Context):
    ...
    if ctx.auth.has_membership('admin'):
        ...
        redirect(ctx.URL('chat'))
    return dict(message = 'Hello and Welcome')


@app.route('chat')
@app.use(ctxd.track_visited, 'chat.html')  # note there is no session, but it used!
def index(ctx: Context):
    user = ctx.auth.user
    flash = ctx.flash

    flash.set('Welcome %s to Websaw Chat. Please be nice to one another' % user['username'], 'info')
    return dict(user=user)

@app.route('start-up')
def index(ctx):
    user = ctx.auth.user
    rooms = db(db.room).select().as_list()
    return dict(rooms=rooms, user= user)
